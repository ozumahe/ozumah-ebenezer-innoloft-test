...
const AddBuisnessModelModal: FC<Props> = ({ addBmRef }: Props) => {
  const dispatch = useDispatch();
  const [isRequest, setIsRequest] = useState<boolean>(false);
  const [technology, setTechnology] = useState<string>("");
  const closeModal = () => {
    addBmRef.current.click();
  };

  const addBuisnessModel = async () => {
    setIsRequest(true);
    try {
  ...
  return (
    <div>
      <input type="checkbox" id="add-bm-modal" className="modal-toggle" />
      <div className="modal">
        <div className="modal-box border-primaryColor border-[2px]">
          ...
          <div className="py-4">
            <input
              ...
              placeholder="Add Buisness Model"
            />
          </div>
          ...
          <div className="modal-action">
            <label htmlFor="add-bm-modal" ref={addBmRef} className="btn hidden">
              Yay!
            </label>
          </div>
        </div>
...
export default AddBuisnessModelModal;
